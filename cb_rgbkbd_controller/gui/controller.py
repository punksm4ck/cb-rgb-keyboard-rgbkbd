...
#             self.logger.error(f"Failed to load settings: {e}")
    ...
    def _update_preview(self):
        ...
        if not self.preview_canvas or not self.preview_enabled_var.get():
            return
    ...
    def _update_hardware_status(self):
        ...
        try:
            status = self.hardware.get_status() if self.hardware else "Unavailable"
            self.hardware_status_label.configure(text=f"Hardware: {status}")
        except Exception as e:
            self.logger.error(f"[HARDWARE STATUS ERROR] {e}")
            self.hardware_status_label.configure(text="Hardware: Error")
...
#             self.logger.error(f"Error changing theme: {e}")
    ...
    def on_key_click(self, event):
        ...
        try:
            key_id = self._get_key_id_from_event(event)
            selected_color = self.color_picker.get_selected_color()
            self.current_colors[key_id] = selected_color
            self._update_preview()
            self.logger.info(f"Key {key_id} updated to {selected_color.to_hex()}")
        except Exception as e:
...
#         self.logger.error(f"Wave effect failed: {e}")
    ...
    def run_pulse_effect(self, base_color=(255, 0, 0), steps=10, delay=0.1):
        ...
        try:
            # Example pulse logic — replace with actual effect code
            for i in range(steps):
                intensity = int(255 * (i / steps))
                color = RGBColor(intensity, 0, 0)
                self.hardware.apply_color(color)
                time.sleep(delay)
            self.logger.info("Pulse effect completed")
        except Exception as e:
    ...
    def run_gradient_effect(self, base_color=(0, 255, 0)):
        ...
        try:
            for i in range(len(self.current_colors)):
                factor = i / max(len(self.current_colors) - 1, 1)
                r = int(base_color[0] * factor)
                g = int(base_color[1] * factor)
                b = int(base_color[2] * factor)
                self.current_colors[i] = RGBColor(r, g, b)

            self._update_preview()
            self.logger.info("Gradient effect applied successfully")
        except Exception as e:
...
class AdvancedEffectControls:
    ...
    def get_current_parameters(self) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.error(f"Error retrieving parameters: {e}")
            return {}
    ...
    def _monitor_loop(self):
        ...
        while not self._stop_monitoring.is_set():
            try:
                ...
                # Optional: add CPU/memory usage tracking here
                # self.performance_data['cpu_usage'] = psutil.cpu_percent()
                # self.performance_data['memory_usage'] = psutil.virtual_memory().percent

                if fps < self.fps_threshold or avg_frame_time > self.frame_time_threshold:
                    self.controller.logger.warning(f"Performance drop: FPS={fps:.2f}, FrameTime={avg_frame_time:.3f}s")
...
#             print(f"Tray notification failed: {e}")
    ...
    def _apply_zone_colors(self):
        """Apply zone colors to hardware"""
        import requests
        try:
            for i, color in enumerate(self.zone_colors):
                # Example: send color to hardware zone i
                self.hardware.set_zone_color(i, color)
            self.logger.info("Zone colors applied successfully")
        except Exception as e:
            self.logger.error(f"Zone color application failed: {e}")
...
#             print(f"Timeline error: {e}")
    ...
    def _apply_group_colors(self):
        """Apply group colors to specified zones"""
        import requests
        try:
            ...
            for name, var in self.group_entries.items():
                ...
                for z in zones:
                    ...
                    if z.isdigit():
                        zone_index = int(z)
                        self.hardware.set_zone_color(zone_index, color)
            self.logger.info("Group colors applied successfully")
        except Exception as e:
            self.logger.error(f"Group color application failed: {e}")
...
#             print(f"Tagging failed: {e}")
    ...
    def _update_tag_autocomplete(self):
        """Update tag autocomplete dropdown based on config files"""
        tags = set()
        try:
            for fname in os.listdir("."):
                if fname.endswith(".json") and "_config_" in fname:
            ...
            self.logger.info("Tag autocomplete updated")
        except Exception as e:
            self.logger.error(f"Failed to update tag autocomplete: {e}")
    ...
    def _rotate_diagnostics_files(self, max_files=5):
        """Delete old diagnostics files beyond max_files"""
        import glob, os
        try:
            ...
            for f in files[max_files:]:
                try:
                    os.remove(f)
                    self.logger.info(f"Deleted old diagnostics file: {f}")
                except Exception as e:
                    self.logger.warning(f"Failed to delete {f}: {e}")
        except Exception as e:
            self.logger.error(f"Diagnostics rotation failed: {e}")
...
def _debug_script(self):
    """Run debug diagnostics without reinitializing GUI"""
    import traceback
    try:
        self.logger.info("Running debug script...")
        self._update_preview()
        self._update_hardware_status()
        self._rotate_diagnostics_files()
        self._auto_export_diagnostics()
        self.logger.info("Debug script completed successfully")
        return True
    except Exception as e:
...
def _debug_script_with_inspector(self):
    """Run debug script and inspect local variables"""
    import traceback
    local_vars = {}
    try:
        self.logger.info("Running debug script with inspector...")

        # Example debug logic — replace with actual diagnostics
        local_vars["preview_enabled"] = self.preview_enabled_var.get()
        local_vars["current_effect"] = self.effect_var.get()
        local_vars["zone_count"] = len(self.zone_colors)
        local_vars["hardware_status"] = self.hardware.get_status() if self.hardware else "Unavailable"
...
def _create_mqtt_listener_tab(self):
    """Create MQTT listener tab in the GUI"""
    self.mqtt_tab = ttk.Frame(self.notebook)
    self.notebook.add(self.mqtt_tab, text="MQTT Listener")

    self._add_help_box(
        self.mqtt_tab,
        "Start an MQTT listener to sync effects from external sources. Incoming messages trigger zone updates."
    )
...
def _create_effect_composer_tab(self):
    """Create Effect Composer tab in the GUI"""
    self.composer_tab = ttk.Frame(self.notebook)
    self.notebook.add(self.composer_tab, text="Effect Composer")

    self._add_help_box(
        self.composer_tab,
        "Use this tab to stack multiple lighting effects as layers. You can drag tracks, blend modes, and export your composition as a .rgbstack preset."
    )
...
def _create_preview_recorder_tab(self):
    """Create Preview Recorder tab in the GUI"""
    self.recorder_tab = ttk.Frame(self.notebook)
    self.notebook.add(self.recorder_tab, text="Preview Recorder")

    self._add_help_box(
        self.recorder_tab,
        "Record your layout canvas as a video or GIF. Adjust FPS and duration, then export to shareable media."
    )
...
def _create_smart_zone_tab(self):
    """Create Smart Zones tab in the GUI"""
    self.smart_tab = ttk.Frame(self.notebook)
    self.notebook.add(self.smart_tab, text="Smart Zones")

    self._add_help_box(
        self.smart_tab,
        "Automatically detect key clusters like WASD or arrows and apply optimized lighting patterns. Click 'Optimize Zones' to apply."
    )
...
def _create_ec_sandbox_tab(self):
    """Create EC Sandbox tab in the GUI"""
    self.sandbox_tab = ttk.Frame(self.notebook)
    self.notebook.add(self.sandbox_tab, text="EC Sandbox")

    self._add_help_box(
        self.sandbox_tab,
        "Test EC commands live in a terminal-style interface. View raw responses and save macros for reuse."
    )
...
def _run_ec_command(self):
    """Run EC command and display output"""
    import subprocess
    cmd = self.ec_entry.get()
    try:
        result = subprocess.run(cmd, shell=True, capture_output=True, text=True)
        output = result.stdout.strip() or result.stderr.strip()
        self.ec_output.insert(tk.END, f"💻 {cmd}\n{output}\n\n")
    except Exception as e:
...
def _create_event_automation_tab(self):
    """Create Event Automation tab in the GUI"""
    self.event_tab = ttk.Frame(self.notebook)
    self.notebook.add(self.event_tab, text="Event Automation")

    self._add_help_box(
        self.event_tab,
        "Create rules that trigger effects based on system events like app launch or USB insert. Use the rule builder and toggle the listener."
    )
...
def _create_copilot_tab(self):
    """Create Copilot tab in the GUI"""
    self.copilot_tab = ttk.Frame(self.notebook)
    self.notebook.add(self.copilot_tab, text="Copilot")

    self._add_help_box(
        self.copilot_tab,
        "Ask Copilot for help with effects, presets, or debugging. Type a question and get real-time orchestration guidance."
    )
...
def _compare_changelogs(self):
    """Compare changelogs between two versions using selected plugin"""
    plugin = self.diff_plugin_var.get()
    va = self.version_a_entry.get()
    vb = self.version_b_entry.get()
    try:
        diff = self.diff_engine.compare(plugin, va, vb)
        self.diff_output.delete("1.0", tk.END)
        self.diff_output.insert(tk.END, f"🔍 Comparing {va} → {vb} with {plugin} plugin:\n\n{diff}")
    except Exception as e:
...
def _highlight_diff(self):
    """Highlight changelog differences between two versions"""
    plugin = self.diff_plugin_var.get()
    va = self.version_a_entry.get()
    vb = self.version_b_entry.get()
    try:
